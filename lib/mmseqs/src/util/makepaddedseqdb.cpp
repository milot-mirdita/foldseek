...
int makepaddedseqdb(int argc, const char **argv, const Command &command) {
    ...
    // A packed two-alphabet DB stores primary * auxAlphabetSize + aux in a single byte. Copy that
    // byte through untouched: masking cannot be baked in here, because the same DB is what the
    // aligner reads back and it needs the real states. Both prefilters mask the primary channel
    // on the fly instead. The extended dbtype bit survives setExtendedDbtype's OR above, so the
    // packed layout stays detectable on the output DB.
    const bool packedAlphabet =
        (DBReader<DBKeyType>::getExtendedDbtype(dbr.getDbtype()) & Parameters::DBTYPE_EXTENDED_AUX_SEQ) != 0;
    // Pad with primary 'X' so the tail scores neutrally, exactly like the unpacked path pads with
    // numeric 20. The aux half of the padding is never read, it only has to stay in range.
...
#pragma omp for schedule(static)
    for (size_t i = 0; i < dbr.getSize(); i++) {
        ...
        char padByte;
        if (packedAlphabet) {
            // seqLen comes from the index, never from newline scanning: a packed byte can be
            // 0x0A. islower()/+32 must not run here either, both would corrupt packed values.
